...
from models.modules.flow_comp_raft import RAFT_bi
from models.recurrent_flow_completion import RecurrentFlowCompleteNet
from models.propainter import InpaintGenerator
from utils.misc import generate_video_from_frames
from utils.misc import to_tensors
...
class InpaintingHandler(BaseHandler):
    def __init__(self, args):
        super().__init__(args)
        self.propainter_ckpt = self.handle_args.get('propainter_ckpt')
        self.raft_ckpt = self.handle_args.get('raft_ckpt')
        self.flow_completion_ckpt = self.handle_args.get('flow_completion_ckpt')
        self.device = self.handle_args.get('device')
        self.use_half = self.handle_args.get('use_half')
    ...
    def init_model(self):
        if self.model is None:
            ...
            ##############################################
            self.fix_raft = RAFT_bi(self.raft_ckpt, self.device)

            self.fix_flow_complete = RecurrentFlowCompleteNet(self.flow_completion_ckpt)
            ...
            ##############################################
            self.model = InpaintGenerator(model_path=self.propainter_ckpt).to(self.device)
            self.model.eval()
    ...
    def inpaint_video(self, video_state, resize_ratio_number, dilate_radius_number, raft_iter_number, 
                      subvideo_length_number, neighbor_length_number, ref_stride_number, mask_dropdown
                      ):
        ...
        # inpaint for videos
        inpainted_frames = self.inpaint(frames, 
                                                    inpaint_masks, 
        ...
        video_output = generate_video_from_frames(inpainted_frames, output_path="./results/inpaint/{}".format(video_state["video_name"]), fps=fps) # import video_input to name the output video
